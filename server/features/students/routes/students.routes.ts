...
export const getStudentInterventions: RequestHandler = (req, res) => {
  try {
    ...
    const sanitizedId = sanitizeString(id);
    const interventions = studentsService.getStudentInterventions(sanitizedId);
...
export const addStudentIntervention: RequestHandler = (req, res) => {
  try {
    ...
    const sanitizedIntervention = {
      ...intervention,
      id: intervention.id ? sanitizeString(intervention.id) : intervention.id,
      studentId: intervention.studentId ? sanitizeString(intervention.studentId) : intervention.studentId,
      date: intervention.date ? sanitizeString(intervention.date) : intervention.date,
      title: intervention.title ? sanitizeString(intervention.title) : intervention.title,
      status: intervention.status ? sanitizeString(intervention.status) : intervention.status,
    };
    
    studentsService.createIntervention(sanitizedIntervention);
