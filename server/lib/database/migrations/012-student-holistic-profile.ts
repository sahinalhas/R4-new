  ...
  up: (db: Database.Database) => {
    ...
    db.exec(`
      CREATE TABLE IF NOT EXISTS student_social_relations (
        ...
        socialSkillsLevel TEXT CHECK (socialSkillsLevel IN ('GELİŞTİRİLMELİ', 'TEMEL', 'YETERLİ', 'İYİ', 'İLERİ')),
        conflictResolutionSkills TEXT CHECK (conflictResolutionSkills IN ('ZAYIF', 'GELİŞMEKTE', 'YETERLİ', 'İYİ')),
        leadershipQualities TEXT,
        teamworkAbility TEXT CHECK (teamworkAbility IN ('ZAYIF', 'ORTA', 'İYİ', 'ÇOK_İYİ')),
    ...
    db.exec(`
      CREATE TABLE IF NOT EXISTS student_future_vision (
        ...
        timeManagementSkills TEXT CHECK (timeManagementSkills IN ('ZAYIF', 'GELİŞMEKTE', 'YETERLİ', 'İYİ')),
